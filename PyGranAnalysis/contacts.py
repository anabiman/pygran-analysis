...
class Neighbors(object):
    ...
    def filter(self, percent=None):
        ...
        if percent:
            ...
            ri, rj = (
                self._Particles.radius[indices[:, 0]],
                self._Particles.radius[indices[:, 1]],
            )
            radii = ri * rj / (ri + rj)

            indices = numpy.unique(
                numpy.array(indices[self._overlaps[:, 0] <= percent * radii, 1:])
            )
    ...
    def findWithin(self, coords, r):
        ...
        )  # self._tree.query_ball_point(coords, r)
