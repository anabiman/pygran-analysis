...
class Neighbors(object):
    ...
    def __init__(self, Particles, material=None, cutoff=None, binary=False):
        ...
        if binary:
            if not hasattr(self._Particles, "type"):
                raise RuntimeError(
                    "Binary Particles object must contain type attribute!"
                )
        ...
        self._overlaps = numpy.zeros(
            (len(self._pairs), 3), dtype=object
        )  # Can we use mixed dtypes for improved mem allocation?
        self._cutoff = cutoff
        ...
        for pair in self._pairs:
            i, j = pair
            self._distances[count] = numpy.sqrt(
                (Particles.x[i] - Particles.x[j]) ** 2.0
            ...
            if self._distances[count] <= Particles.radius[i] + Particles.radius[j]:
                self._overlaps[count] = numpy.array(
                    [
                        Particles.radius[i]
                        + Particles.radius[j]
                        - self._distances[count],
                        i,
                        j,
                    ],
                    dtype=object,
        ...
        self._overlaps = numpy.array(
            self._overlaps, dtype=object
        )  # Can we use mixed dtypes for improved mem allocation?
        ...
        if material:
            ...
            self._model = None  # SpringDashpot(**self._material)
    ...
    def coon(self, type1=1, type2=2):
        ...
        if self._binary:
            ...
            typeA, typeB = (
                numpy.zeros(Na, dtype="int64"),
                numpy.zeros(Nb, dtype="int64"),
            )
            count = 0
                ...
                if self._Particles[i].type == type1:
                    if hasattr(self._Particles, "mol"):
                        if partsB.mol.max() > 0:
                            ...
                            if type1 == type2:
                                typeA[count] = (
                                    sum(self._Particles[cn].type == type2) - 1
                                )
                            else:
                ...
                if self._Particles[i].type == type2:
                    if hasattr(self._Particles, "mol"):
                        if partsA.mol.max() > 0:
                            ...
                            if type1 == type2:
                                typeB[count] = (
                                    sum(self._Particles[cn].type == type1) - 1
                                )
                            else:
            ...
            if hasattr(self._Particles, "mol"):
                if partsA.mol.max() > 0:
                    ...
                    for i, partA in enumerate(partsA):
                        tmpA[int(partA.mol) - 1] += typeA[
                            i
                        ]  # mols are numbered from 1 ... N in LAMMPS
            ...
            if hasattr(self._Particles, "mol"):
                if self._Particles.mol[0] > 0:
                    ...
                    for i, cn in enumerate(self._neigh):
                        coon[int(self._Particles.mol[i])] += (
                            len(numpy.unique(self._Particles[cn].mol)) - 1
                        )
    ...
    def filter(self, percent):
        ...
        if percent:
            for count, pair in enumerate(self._pairs):
                ...
                if (
                    Particles.radius[i] + Particles.radius[j] - self._distances[count]
                    > percent * reff
                ):
    ...
    def forceChain(
        ...
        stress_prin = numpy.zeros(
            (self._Particles.natoms, 3)
        )  # 2 stress components + angle = 4 dims
        ...
        for contact in self._overlaps:
            ...
            overlap = (
                self._Particles.radius[i]
                + self._Particles.radius[j]
                + numpy.array([x[i] - x[j], y[i] - y[j]])
            )
            ...
            stress_i = (
                numpy.outer(
                    self._model.normalForce(overlap, self._Particles.radius[i]), overlap
                )
                / vi
            )
            stress_j = (
                numpy.outer(
                    self._model.normalForce(overlap, self._Particles.radius[j]), overlap
                )
                / vj
            )
        ...
        for i in range(self._Particles.natoms):
            ...
            stress_p = 0.5 * (stress_i[0, 0] + stress_i[1, 1]) + numpy.sqrt(
                (0.5 * (stress_i[0, 0] - stress_i[1, 1])) ** 2.0 + stress_i[0, 1]
            )

            if (
                stress_i[0, 0] - stress_i[1, 1]
            ):  # otherwise this is prolly a boundary particle

                # Compute angle
                stress_prin[i, 2] = 0.5 * numpy.arctan(
                    2.0 * stress_i[0, 1] / (stress_i[0, 0] - stress_i[1, 1])
                )
        ...
        if not peters:  # plot all particle stresses
            ...
            indices = numpy.where(stress_norm >= threshold * stress_norm.mean(axis=0))[
                0
            ]
            ...
            for nns in self._nns:
                if (
                    len(nns) <= 2
                ):  # select only particles surrounded by 2 or more highly stressed particles
                    remInd.append(nns)
                        ...
                        if (term3 <= term4) and (cosAlpha * term4) < term3:
                            ...
                            if plot_stress:
                                plt.plot(
                                    ...
                                    y,
                                    linewidth=max(
                                        1.2 * stress_norm[i] / stress_norm.max(), 0.01
                                    ),
                                    color="black",
                                )
            ...
            print(
                "Natoms highly stressed = {} out of {} particles".format(
