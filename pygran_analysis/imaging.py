...
def slice(
    ...
    if not Image:
        raise ModuleNotFoundError(
            f"PIL pkg must be installed to use this routines: {__name__}"
        )
                        ...
                        if (ic) ** 2 + (jc) ** 2 <= radius**2:
                            if (
                                ...
                                pixels[i + ic, j + jc] = (
                                    ...
                                    255,
                                    255,
                                )  #  add trans[n] for transparency (e.g. png files) and then set the colour accordingly
                            else:
                                pass
            if imgShow:
                img.show()
        ...
        if file.endswith("*"):
            ...
            if len(np.array(pic.getdata()).shape) > 1:
                data = np.array(pic.getdata()).reshape(
                    n, m, np.array(pic.getdata()).shape[-1]
                )
            else:
    ...
    def func(file):
        for i, img in enumerate(file):
            ...
            if i == 0:
                if len(np.array(pic.getdata()).shape) > 1:
                    data = np.zeros(
                        (n, m, np.array(pic.getdata()).shape[-1], len(file))
                    )
                else:
                    data = np.zeros((n, m, len(file)))

            if len(np.array(pic.getdata()).shape) > 1:
                data[:, :, :, i] = np.array(pic.getdata()).reshape(
                    n, m, np.array(pic.getdata()).shape[-1]
                )
            else:
...
def scaleSegregation(
    ...
    for dist in np.arange(0, maxDist, incr):
        ...
        while nTrials < samplesize:
            ...
            theta, phi = (
                np.arccos(1.0 - 2 * np.random.rand()),
...
def scaleSegregation_fft(
    ...
    N = 2 * len(a) - 1
    corrfunc = (
        np.real(np.fft.ifft(np.fft.fft(a, N) * np.conj(np.fft.fft(a, N)))) / var
    )[: len(a)]
