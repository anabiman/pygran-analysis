...
these properties. This class is iterable but NOT an iterator.
    ...
    def _initialize(self, **args):
        ...
        if type(self).__name__ in args:  # object already exists
            ...
            if hasattr(args[type(self).__name__], "data"):
                ...
                raise IOError(
                    "data attribute not found in {} object".format(type(self).__name__)
                )
            ...
            if "data" in args:
                ...
                if "units" in args:
                    ...
                    raise RuntimeError(
                        "units must be supplied when creating SubSystem from a data dictionary."
                    )
            ...
            if self._fname:
                if (
                    ...
                    if self._vtk == "polydata":
                        ...
                        raise TypeError(
                            "File format {} not understood".format(self._vtk)
                        )
        ...
        if not hasattr(self, "data"):
            ...
            )  # an ordered dict that contains either arrays
    ...
    def __add__(self, obj):
        ...
        if type(obj) is type(self):
            ...
            raise RuntimeError(
                "Cannot add two objects of different types: {} and {}".format(
                    type(obj), type(self)
                )
            )
        ...
        if type(obj) is tuple:
            ...
            if type(obj) is type(self):
                if att == "all":
                    ...
                    self
                ):  # gotta make sure both classes being subtracted have the same number of elements
                    self.data[att] -= obj.data[att]
                else:
                    print(
                        "Two subsystems with different number of elements cannot be subtracted."
                    )
    ...
    def __mul__(self, obj):
        ...
        if type(obj) is not type(self):
            raise RuntimeError(
                "Two subsystems with different types cannot be multiplied."
            )

        if len(obj) != len(
            self
        ):  # gotta make sure both classes being multiplied have the same number of elements
            raise RuntimeError(
                "Two subsystems with different number of elements cannot be multiplied."
            )
        ...
        if type(obj) is tuple:
            ...
            if type(obj) is type(self):
                if att == "all":
                    ...
                    self
                ):  # gotta make sure both classes being divided have the same number of elements
                    self.data[att] /= obj.data[att]
                else:
                    print(
                        "Two subsystems with different number of elements cannot be divided."
                    )
    ...
    def translate(self, value, attr=("x", "y", "z")):
        ...
        if len(value) != len(attr):
            raise ValueError(
                "The length of values must be equal to that of attributes."
            )
...
class Mesh(SubSystem):
    ...
    def _initialize(self, **args):
            ...
            if cells:
                ...
                np_pts = numpy.zeros(
                    (
                        ...
                        self.data[points.GetName()].shape[1],
                    )
                )
                ...
                for i in range(self.nCells()):
                    if "CellVol" in self.data:
                        ...
                        self.data["CellVol"][i] = (
                            (np_pts[i][:, 0].max() - np_pts[i][:, 0].min())
                            * (np_pts[i][:, 1].max() - np_pts[i][:, 1].min())
                            * (np_pts[i][:, 2].max() - np_pts[i][:, 2].min())
                        )
        ...
        while True:
            ...
            if key:
                ...
                newkey = "cells." + key
                self.data[newkey] = vtk_to_numpy(
                    self._output.GetCellData().GetArray(key)
                )

                if args["avgCellData"]:
                    if "CellArea" in self.data:
                        self.data[newkey] = (
                            self.data["CellArea"] * self.data[newkey].T
                        ).T.sum(axis=0) / self.data["CellArea"].sum()

                    elif "CellVol" in self.data:
                        self.data[newkey] = (
                            self.data["CellVol"] * self.data[newkey].T
                        ).T.sum(axis=0) / self.data["CellVol"].sum()
        ...
        while True:
            ...
            if key:
                newkey = "points." + key
                self.data[newkey] = vtk_to_numpy(
                    self._output.GetPointData().GetArray(key)
                )
                index += 1
            else:
                break
    ...
    def _updateSystem(self):
        ...
        self._initialize(
            **self._args
        )  # it's imp to pass args so that any subsystem-specific args are
...
class Particles(SubSystem):
            ...
            if hasattr(self, "_fname"):
                if self._fname:
                    ...
                    if (
                        self._ftype == ".dump"
                        or self._ftype == ".data"
                        or self._ftype == ".lammps"
                    ):  # need a better way to figure out this is a LIGGGHTS/LAMMPS file

                        if "*" in self._fname:
                            self._files = sorted(
                                glob.glob(self._fname), key=numericalSort
                            )
                        ...
                        raise IOError(
                            "Input trajectory must be a valid LAMMPS/LIGGGHTS (dump) or vtk file."
                        )
    ...
    def computeRDF(self, dr=None, center=True, rMax=None, npts=100, optimize=False):
        ...
        print(
            "Constructing a cube of length {} and a circumscribed sphere of radius {}".format(
                S * 2.0, rMax
            )
        )
    ...
    def computeIntensitySegregation(self, resol=None):
        ...
        for sn, ctype in enumerate(numpy.unique(ptypes)):
            ...
            parts.translate(
                value=(-parts.x.min(), -parts.y.min(), -parts.z.min()),
                attr=("x", "y", "z"),
            )
    ...
    def _goto(self, current_frame, go_frame):
        ...
        if self._singleFile:
            ...
            if current_frame == go_frame:
                ...
                raise ValueError(
                    "Cannot find frame {} in current trajectory".format(current_frame)
                )
    ...
    def _writeDumpFile(self, filename):
        ...
        with open(filename, "a") as fp:
            ...
            if "box" not in self.data:
                ...
                self.data["box"] = (
                    [self.data["x"].min() - maxR, self.data["x"].max() + maxR],
    ...
    def _readFile(self, frame):
        ...
        if self._singleFile:  # must support skip
            if self._ftype == ".dump":
                ...
                raise IOError(
                    "{} format is not a supported trajectory file.".format(self._ftype)
                )
        else:
            if self._ftype == ".dump":
    ...
    def factory(**args):
        ...
        for ss in args:
            ...
            if not sclass:
                ...
                raise ValueError(
                    "System takes only keywords of classes that are defined. Class type {} not found.".format(
                        args[ss]
                    )
                )
                ...
                for item in args[ss]:
                    ...
                    if isinstance(item, tuple):  # we must pass additional args
                        ...
                        try:
                            ...
                            raise ValueError(
                                "SubSystem arguments must be passed as a dictionary, i.e. System(SubSystem=[(path1, arg1), ...]) "
                            )
                        else:
                        ...
                        raise ValueError(
                            "Incorrect keyword supplied to System: {} when creating {} SubSystem".format(
                                item, ss
                            )
                        )
...
def select(data, *region):
    ...
    try:
        if not len(region):
            ...
            if len(region) != 6:
                print(
                    "Length of region must be 6: (xmin, xmax, ymin, ymax, zmin, zmax)"
                )
                raise
