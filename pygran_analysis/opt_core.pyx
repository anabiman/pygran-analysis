...
cdef void histogram(
    ...
    bin_counts_ptr[0] = <int*> stdlib.calloc(bin_count, sizeof(int))

    for i in range(data_count):
        bin = which_bin(data[i], incr)
        if bin > 0 and bin < bin_count:
            bin_counts_ptr[0][bin] += 1
            #print("bin, count = ", bin, bin_counts_ptr[0][bin])
...
def computeRDF(numpy_x, numpy_y, numpy_z, dr, S, rMax):
    ...
    cdef int nindices = cython_computeRDF(numpy_x, numpy_y, numpy_z, dr, S, rMax, min, max, nparts, num_increments, &g_average, &radii, &interior_indices, edges)
...
cdef int cython_computeRDF(
        ...
        int nparts,
        int num_increments,
        double** g_average_dptr,
        double** radii_dptr,
        int** interior_indices_dptr,
        double[] edges
    ):
    ...
    g_average = g_average_dptr[0]
    radii = radii_dptr[0]
    interior_indices = interior_indices_dptr[0]
    ...
    for p in range(num_interior_particles):
        ...
        d[index] = 2.0 * rMax
        histogram(d, nparts, edges, num_increments, dr, &result)
